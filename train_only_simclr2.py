...
def train_one_epoch(epoch,
    ...
    for it in range(n_iters):
        ...
        # --------------------------------------
        imgs = torch.cat([ims_u_weak, ims_u_strong], dim=0).cuda()
        lbs_u_real = torch.cat([lbs_u_real, lbs_u_real], dim=0).cuda()
        imgs = interleave(imgs, 2 * mu)
        logits, logit_z = model(imgs)
        logits_z = de_interleave(logit_z, 2 * mu)
        logits = de_interleave(logits, 2 * mu)

        logits_u_w_z, logits_u_s_z = torch.split(logits_z, bt * mu)

        # entrenar primero con simclr el espacio h de las imagenes separadas
        loss_simCLR = (criteria_z(logits_u_w_z, logits_u_s_z))

        # with torch.no_grad():
        # print(logits.shape, lbs_u_real.shape)
        loss_u_real = F.cross_entropy(logits, lbs_u_real).mean()
        loss = loss_simCLR + loss_u_real
        ...
        if (it + 1) % 64 == 0:
            ...
            logger.info("epoch:{}, iter: {}. loss: {:.4f}. loss_u_real: {:.4f}. "
                        " loss_simclr: {:.4f}. LR: {:.4f}. Time: {:.2f}".format(
                epoch, it + 1, loss_meter.avg, loss_u_real_meter.avg,
                loss_simclr_meter.avg, lr_log, t))

            epoch_start = time.time()

    ema.update_buffer()
    return loss_u_real_meter.avg, loss_simclr_meter.avg
...
def main():
    ...
    for epoch in range(args.n_epoches):
        loss_u_real, loss_simclr = train_one_epoch(epoch, **train_args)
        # torch.cuda.empty_cache()

        top1, top5, valid_loss = evaluate(ema, dlval, criteria_x)

        writer.add_scalars('train/1.loss', {'train': loss_u_real,
